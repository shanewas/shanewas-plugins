"""minimal-diff packaging tests: manifests, hooks, skill, and tool presence.

Mirrors the slop-gate fileset contract: the three native manifests parse,
versions match the root VERSION, the SKILL.md description opens with a
Use-when trigger list, and every shipped hook file carries a valid
envelope pointing at tools/diffgate.py.
"""
...
from frontmatter import ROOT, read_frontmatter
...
def _load_json(rel):
    path = PLUGIN / rel
    assert path.is_file(), "minimal-diff: missing %s" % rel
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise AssertionError("minimal-diff %s: invalid JSON: %s" % (rel, exc))


def test_manifests_parse_with_name_and_version():
    for rel in MANIFEST_FILES:
        data = _load_json(rel)
        assert data.get("name"), "minimal-diff %s missing 'name'" % rel
        assert data.get("version"), "minimal-diff %s missing 'version'" % rel
...
def test_manifest_versions_match_root():
    ...
    for rel in MANIFEST_FILES:
        ...
        assert data.get("version") == root_version, (
            "minimal-diff %s version %r != root VERSION %r"
            % (rel, data.get("version"), root_version))


def test_tool_and_shim_present():
    tool = PLUGIN / "tools" / "diffgate.py"
    assert tool.is_file(), "minimal-diff: missing tools/diffgate.py"
    ...
    assert (PLUGIN / "plugin.js").is_file(), "minimal-diff: missing plugin.js"
    shim = (PLUGIN / "plugin.js").read_text(encoding="utf-8")
    assert "tools/diffgate.py" in shim or "tools\", \"diffgate.py" in shim, (
        "plugin.js must call tools/diffgate.py")
...
def test_claude_hooks_envelope():
    ...
    assert any("tools/diffgate.py" in cmd for cmd in commands), (
        "hooks/hooks.json must route to tools/diffgate.py, got %r" % (commands,))
...
def test_codex_hooks_envelope():
    ...
    commands = [hook.get("command", "")
                for entry in post for hook in entry.get("hooks", [])]
    assert any("./tools/diffgate.py" in cmd for cmd in commands), (
        "hooks.json must call ./tools/diffgate.py relative to the plugin root, "
        "got %r" % (commands,))


def test_antigravity_snippet_shape():
    data = _load_json("hooks/antigravity-hooks.json")
    entry = data.get("minimal-diff")
    assert entry, "antigravity snippet lacks the minimal-diff entry"
...
def test_skill_description_use_when():
    skill = PLUGIN / "skills" / "minimal-diff" / "SKILL.md"
    assert skill.is_file(), "minimal-diff: missing skills/minimal-diff/SKILL.md"
    fields, _ = read_frontmatter(skill)
    assert fields.get("name") == "minimal-diff", (
        "SKILL.md name %r must match its directory" % fields.get("name"))
    description = fields.get("description", "")
    assert description.startswith("Use when"), (
        "SKILL.md description must open with 'Use when', got %r"
        % description[:60])
