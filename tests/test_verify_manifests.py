"""verify-done packaging tests: manifests, hooks, and tool presence.
...
and every shipped hook file carries a valid envelope pointing at tools/donecheck.py.
"""
...
PLUGIN = ROOT / "plugins" / "verify-done"
MANIFEST_FILES = (
...
def _load_json(rel):
    path = PLUGIN / rel
    assert path.is_file(), "verify-done: missing %s" % rel
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise AssertionError("verify-done %s: invalid JSON: %s" % (rel, exc))


def test_manifests_parse_with_name_and_version():
    for rel in MANIFEST_FILES:
        data = _load_json(rel)
        assert data.get("name"), "verify-done %s missing 'name'" % rel
        assert data.get("version"), "verify-done %s missing 'version'" % rel
...
def test_manifest_versions_match_root():
    ...
    for rel in MANIFEST_FILES:
        ...
        assert data.get("version") == root_version, (
            "verify-done %s version %r != root VERSION %r"
            % (rel, data.get("version"), root_version))


def test_tool_and_shim_present():
    tool = PLUGIN / "tools" / "donecheck.py"
    assert tool.is_file(), "verify-done: missing tools/donecheck.py"
    ...
    assert (PLUGIN / "plugin.js").is_file(), "verify-done: missing plugin.js"
    shim = (PLUGIN / "plugin.js").read_text(encoding="utf-8")
    assert "tools/donecheck.py" in shim or "tools\", \"donecheck.py" in shim, (
        "plugin.js must call tools/donecheck.py")
...
def test_claude_hooks_envelope():
    ...
    assert any("tools/donecheck.py" in cmd for cmd in commands), (
        "hooks/hooks.json must route to tools/donecheck.py, got %r" % (commands,))
...
def test_codex_hooks_envelope():
    ...
    commands = [hook.get("command", "")
                for entry in post for hook in entry.get("hooks", [])]
    assert any(cmd.startswith("python3 ./tools/donecheck.py") for cmd in commands), (
        "hooks.json must call ./tools/donecheck.py relative to the plugin root, "
        "got %r" % (commands,))


def test_antigravity_snippet_shape():
    data = _load_json("hooks/antigravity-hooks.json")
    entry = data.get("verify-done")
    assert entry, "antigravity snippet lacks the verify-done entry"
    ...
    assert any("tools/donecheck.py" in cmd for cmd in commands), (
        "antigravity snippet must route to tools/donecheck.py")


def test_muse_tbd_note_present():
    note = PLUGIN / "docs" / "muse-hooks-TBD.md"
    assert note.is_file(), "verify-done: missing docs/muse-hooks-TBD.md"
    body = note.read_text(encoding="utf-8")
    assert "donecheck.py" in body, "Muse TBD note must name the CLI fallback"
